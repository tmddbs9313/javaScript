...
/* 함수의 매개변수를 통해 다른 함수의 내부로 전달되는 함수를 "콜백 함수"라고 한다. 
    매개변수를 통해 함수의 외부에서 콜백함수를 전달받은 함수는 "고차함수"라고 한다.
    ...
    function apply(func, value) {
        ...
        //고차함수는 매개변수를 통해 전달 받은 콜백함수의 호출 시점을 결정하여 호출한다.
        //콜백함수는 고차함수에 의해 호출(헬파)되며, 이때 고차함수는
        //필요에 따라 콜백함수에 인수를 전달할 수 있다;
        return func(value);
    } 
    ...
    //콜백함수를 익명함수 리터럴로 정의하면 곧바로 고차함수에 전달할 수 있다.
    console.log(apply(function(value){return value ** 2;}, 5));  // ** = 제곱

    /* 함수의 변하지 않는 공통로직은 미리 정의해두고 경우에 따라 변경되는 로직은 
